...
export function getSystemPrompt(config: Config, userMemory?: string | null, tools?: Tool[]): string {
  const parts: string[] = [];
...
function getToolGuidelinesSection(tools: Tool[]): string {
  const regularTools = tools.filter((tool) => !tool.name.startsWith("subagent_"));
  const subagentTools = tools.filter((tool) => tool.name.startsWith("subagent_"));
  ...
  for (const tool of regularTools) {
    const description = tool.description.length > 100 ? `${tool.description.slice(0, 100)}...` : tool.description;
